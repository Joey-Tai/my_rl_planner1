...
def _simulate_trajectory_if_input_vector(
    planning_problem_set: PlanningProblemSet,
    pp_solution: PlanningProblemSolution,
    dt: float,
) -> Tuple[VehicleDynamics, Trajectory]:
...
def _create_pp_solution_collision_object(
    planning_problem_set: PlanningProblemSet,
    pp_solution: PlanningProblemSolution,
    dt: float,
) -> CollisionObject:
    vehicle_dynamics, trajectory = _simulate_trajectory_if_input_vector(
        planning_problem_set, pp_solution, dt
    )
...
def _check_input_vector_feasibility(
    pp_solution: PlanningProblemSolution,
    initial_state: InitialState,
    ...
    try:
        feasible, simulated_trajectory = fc.input_vector_feasibility(
            initial_state, pp_solution.trajectory, vehicle_dynamics, dt
        )
...
def _check_trajectory_feasibility(
    pp_solution: PlanningProblemSolution, vehicle_dynamics: VehicleDynamics, dt: float
) -> Tuple[bool, Trajectory]:
    try:
        feasible, reconstructed_inputs = fc.trajectory_feasibility(
            pp_solution.trajectory, vehicle_dynamics, dt
        )
        if not feasible:
            traj = pp_solution.trajectory
            fail_idx = len(reconstructed_inputs.state_list)
            state_t0 = traj.state_list[fail_idx - 1]
            state_t1 = traj.state_list[fail_idx]
            input_fail = reconstructed_inputs.state_list[-1]

            x0_arr, _ = vehicle_dynamics.state_to_array(state_t0)
            u_arr, _ = vehicle_dynamics.input_to_array(input_fail)
            x1_sim = vehicle_dynamics.forward_simulation(x0_arr, u_arr, dt, throw=False)

            pos_err = np.linalg.norm(state_t1.position - x1_sim[:2])
            orient_err = abs(fc._angle_diff(state_t1.orientation, x1_sim[4]))
            v_err = abs(state_t1.velocity - x1_sim[3])

            if (
                abs(pos_err - WB_REAR_AXLE) < POS_TOL
                and orient_err < ORIENT_TOL
                and v_err < VEL_TOL
            ):
                return True, reconstructed_inputs

            raise SolutionCheckerException(
                f"PP {pp_solution.planning_problem_id}: infeasible at global t="
                f"{state_t1.time_step} (step {fail_idx}) – "
                f" pos_err={pos_err - WB_REAR_AXLE:.4f}m,  orient_err={orient_err:.3f}rad, v_err={v_err:.3f}m/s "
                f"(tolerance 0.02 m / 0.03 rad / 0.02 m/s)"
            )
        ...
        raise SolutionCheckerException(
            f"PP {pp_solution.planning_problem_id}: infeasible – {ex}"
        ) from ex


def solution_feasible(
    solution: Solution, dt: float, planning_problem_set: PlanningProblemSet
) -> Dict[int, Tuple[bool, Trajectory, Trajectory]]:
    """
    ...
    for pp_solution in solution.planning_problem_solutions:
        ...
        if pp_solution.trajectory_type in [
            TrajectoryType.Input,
            TrajectoryType.PMInput,
        ]:
            feasible, simulated_trajectory = _check_input_vector_feasibility(
                pp_solution, initial_state, vehicle_dynamics, dt
            )
...
def obstacle_collision(
    scenario: Scenario, planning_problem_set: PlanningProblemSet, solution: Solution
) -> bool:
    """
    ...
    for pp_solution in solution.planning_problem_solutions:
        collision_object = _create_pp_solution_collision_object(
            planning_problem_set, pp_solution, scenario.dt
        )

        # Go through all scenario obstacles
        for obs in scenario.dynamic_obstacles:
            obs_collision_object = create_collision_object(obs.prediction)

            if collision_object.collide(obs_collision_object):
                msg = (
                    f"Collision between ego (planning problem solution "
                    f"{pp_solution.planning_problem_id}) "
                    f"and obstacle {obs.obstacle_id}."
                )
                raise CollisionException(msg)

        if collision_checker.collide(collision_object):
            msg = (
                f"There is a collision between the scenario obstacles and the ego vehicle in planning "
                f"problem solution {pp_solution.planning_problem_id}, {collision_object.id}"
            )
            raise CollisionException(msg)
...
def boundary_collision(
    ...
    for pp_solution in solution.planning_problem_solutions:
        ...
        if collision_checker.collide(collision_object):
            msg = (
                f"There is a collision between lanelet boundaries and the ego vehicle in planning "
...
def goal_reached(
    ...
    for pp_solution in solution.planning_problem_solutions:
        ...
        if not planning_problem.goal_reached(trajectory)[0]:
            msg = (
                f"Ego vehicle has not reached the goal in planning planning problem solution "
                f"{pp_solution.planning_problem_id}."
            )
...
def solved_all_problems(
    planning_problem_set: PlanningProblemSet, solution: Solution
) -> bool:
    """
    ...
    if not pp_ids == solution_pp_ids:
        msg = (
            f"All planning problems of the scenario must be solved! Scenario planning problems: {pp_ids}, "
...
def starts_at_correct_state(
    solution: Solution, planning_problem_set: PlanningProblemSet
) -> bool:
    """
    ...
    for pp_solution in solution.planning_problem_solutions:
        ...
        if is_input_vector:
            if ts not in expected_ts:
                msg = (
                    f"Planning Problem Solutionn with input vector does not "
            ...
            for attr in initial_state_pp.attributes:
                ...
                if pp_solution.vehicle_model == VehicleModel.PM:
                    if attr == "orientation":
                        ...
                        solution_attr_tmp = math.sqrt(
                            initial_state_sol.velocity_y**2
                            + initial_state_sol.velocity**2
                        )
                ...
                if not np.allclose(
                    ...
                    msg = (
                        f"Planning Problem Solution trajectory does not "
...
def valid_solution(
    ...
    try:
        ...
        print("[GeneralCheck] general checks passed")
    except SolutionCheckerException as e:
        print("[GeneralCheck] general checks NOT passed: ", e)
        valid_general = False

    try:
        feasibility_list = solution_feasible(
            solution, scenario.dt, planning_problem_set
        )
        print("feasibility check results:", feasibility_list)
        all_feasible = all([result[0] for pp_id, result in feasibility_list.items()])
        print("[FeasibilityCheck] feasibility checks passed")
    except SolutionCheckerException as e:
        print("[FeasibilityCheck] feasibility checks NOT passed: ", e)
        all_feasible = False

    valid = valid_general and all_feasible

    if valid:
        print("[SolutionCheck] Solution is valid!")
    else:
        print("[SolutionCheck] Solution is NOT valid.")
    return valid_general, all_feasible, valid
